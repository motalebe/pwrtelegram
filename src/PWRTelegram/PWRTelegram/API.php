    ...
    public function upload($file, $whattype = 'url', $name = '', $type = '', $forcename = false, $oldparams = [])
    {
        ...
        if (file_exists($file) && $whattype == 'file') {
            ...
            if ($type != $info['file_type'] || $name != '') {
                ...
                shell_exec('wget -r -np -nc --restrict-file-names=nocontrol -qQ 1610612736 -O '.escapeshellarg($path).' '.escapeshellarg($this->pwrtelegram_storage.urlencode($downloadres['result']['file_path'])));
                if (!file_exists($path)) {
                    return ['ok' => false, 'error_code' => 400, 'description' => "Couldn't download file."];
                }
                $size = filesize($path);
